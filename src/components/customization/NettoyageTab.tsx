...
export const NettoyageTab = () => {
  ...
  const mappingFrequence: Record<string, string> = {
    'QUOTIDIEN': 'lbl_quotidien',
    'HEBDOMADAIRE': 'lbl_hebdomadaire',
    'MENSUEL': 'lbl_mensuel',
    'TRIMESTRIEL': 'lbl_trimestriel',
    'APRÈS SERVICE': 'lbl_apres_service',
    'APRÈS UTILISATION': 'lbl_apres_utilisation'
  };

  const handleEdit = (task: NettoyageTaskConfig) => {
    setEditingId(task.id);
    setNom(task.nom);
    setFrequence(task.frequence);
    // @ts-ignore
  ...
  const handleSave = () => {
    ...
    if (editingId === 'NEW') {
      ...
      newNettoyage = nettoyage.map((task: any) => task.id === editingId ? payload : task);
    }
  ...
  const handleDelete = (id: string) => {
    const newNettoyage = nettoyage.filter((task: any) => task.id !== id);
    updateConfig({ nettoyage: newNettoyage });
    setTaches(newNettoyage);
  ...
  const handleSelectAll = () => {
    if (selectedIds.length === nettoyage.length) {
      ...
      setSelectedIds(nettoyage.map(task => task.id));
    }
  };
  ...
  const handleBulkDelete = () => {
    ...
    if (showBulkDeleteConfirm === 'selected') {
      newNettoyage = nettoyage.filter(task => !selectedIds.includes(task.id));
    }
    updateConfig({ nettoyage: newNettoyage });
    setTaches(newNettoyage);
    setSelectedIds([]);
  ...
  return (
    <div className="space-y-6 max-w-2xl pb-20">
      <div className="flex items-center justify-between">
        <h3 className="text-xl font-black text-gray-800">{t('lbl_cleaning_plan_count', { count: nettoyage.length })}</h3>
        {!editingId && (
          <Button onClick={handleCreate} className="bg-crousty-purple text-white gap-2 rounded-xl h-10 px-4">
            <Plus size={16} /> {t('btn_add_task')}
          </Button>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[1000] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] p-8 max-w-sm w-full shadow-2xl space-y-6 animate-in zoom-in-95 duration-300">
            <div className="text-center space-y-2">
              <h3 className="text-xl font-black text-gray-900 leading-tight">{t('lbl_confirmation')}</h3>
              <p className="text-sm text-gray-500 font-medium">
                {showBulkDeleteConfirm === 'all' 
                  ? t('lbl_confirm_empty_cleaning')
                  : t('lbl_confirm_delete_task_count', { count: selectedIds.length })}
              </p>
            </div>
            <div className="flex gap-2">
              <Button onClick={() => setShowBulkDeleteConfirm(null)} variant="ghost" className="flex-1 font-black text-[10px] uppercase">{t('btn_cancel')}</Button>
              <Button onClick={handleBulkDelete} className="flex-1 bg-red-500 font-black text-[10px] uppercase">{t('btn_delete')}</Button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="space-y-3">
        {editingId === 'NEW' && (
          <div className="border-2 border-emerald-500 rounded-2xl bg-white p-6 space-y-5 shadow-xl shadow-emerald-100 mb-6 animate-in zoom-in-95">
            <h4 className="font-black text-xl text-gray-800">{t('lbl_new_task')}</h4>
            <div className="space-y-4">
              <div>
                <Label className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 block">{t('lbl_equipment_name')}</Label>
                <Input value={nom} onChange={e => setNom(e.target.value)} placeholder={t('ph_cleaning_task')} className="h-12 font-bold" />
              </div>
              
              <div>
                <Label className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 block">{t('lbl_frequency')}</Label>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {frequencesEnum.map((f) => {
                    const isSelected = frequence === f;
                    const key = mappingFrequence[f];
                    return (
                      <button
                        key={f}
              ...
              <div>
                <Label className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 block">{t('lbl_instructions_opt')}</Label>
                <textarea
                  value={instructions}
                  onChange={e => setInstructions(e.target.value)}
                  placeholder={t('ph_cleaning_instructions')}
            ...
            <div className="flex gap-3 pt-2">
              <Button variant="outline" className="flex-1 h-12" onClick={() => setEditingId(null)}>{t('btn_cancel')}</Button>
              <Button onClick={handleSave} className="flex-1 h-12 bg-emerald-600 border-none">{t('btn_create')}</Button>
            </div>
          </div>
        )}
        ...
        {nettoyage.map((task) => (
          <div 
            key={task.id} 
            onClick={() => isSelectionMode && toggleSelection(task.id)}
            className={cn(
              "border rounded-2xl bg-white overflow-hidden shadow-sm hover:shadow-md transition-all cursor-pointer",
              !task.actif && "opacity-50 grayscale",
              editingId === task.id ? "border-emerald-500 shadow-lg shadow-emerald-50" : "border-gray-100",
              selectedIds.includes(task.id) && "border-amber-400 bg-amber-50/20"
            )}
          >
            {editingId === task.id ? (
              <div className="p-6 bg-gray-50 space-y-5 animate-in slide-in-from-top-2" onClick={e => e.stopPropagation()}>
                <h4 className="font-black text-gray-800">{t('lbl_edit_task')}</h4>
                <div className="space-y-4">
                  <div>
                    <Label className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 block">{t('lbl_equipment_name')}</Label>
                ...
                <div className="flex gap-3 pt-2">
                  <Button variant="outline" className="flex-1 h-12" onClick={() => setEditingId(null)}>{t('btn_cancel')}</Button>
                  <Button onClick={handleSave} className="flex-1 h-12 bg-emerald-600 border-none text-white">{t('btn_save')}</Button>
                </div>
              </div>
            ) : (
              <div className="flex items-center justify-between p-4 px-6">
                <div className="flex items-center gap-4">
                  {isSelectionMode && (
                    <div className={cn(
                      ...
                      selectedIds.includes(task.id) ? "bg-amber-500 border-amber-500" : "border-gray-200 bg-white"
                    )}>
                  ...
                  <div>
                    <h4 className="font-black text-gray-800 text-lg leading-tight">{task.nom}</h4>
                    <div className="flex items-center gap-2 mt-1">
                      <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full uppercase tracking-widest">
                        {mappingFrequence[task.frequence] ? t(mappingFrequence[task.frequence]) : task.frequence}
                      </span>
                      {task.instructions && (
                ...
                {!isSelectionMode && (
                  <div className="flex items-center gap-1">
                    <button onClick={(e) => { e.stopPropagation(); handleEdit(task); }} className="p-2 text-gray-400 hover:text-crousty-purple hover:bg-purple-50 rounded-xl transition-all">
                      <Edit2 size={20} />
                    </button>
                    <button onClick={(e) => { e.stopPropagation(); setConfirmDelete(task.id); }} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all">
                      <Trash2 size={20} />
                    </button>
                  </div>
                )}
        ...
        {confirmDelete && (
          <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl animate-in zoom-in-95 duration-200">
              ...
              <h3 className="text-xl font-black text-gray-800 text-center mb-2">{t('lbl_delete_task')}</h3>
              <p className="text-gray-500 text-center font-medium mb-6">
                {t('lbl_confirm_delete_item', { item: nettoyage.find(task => task.id === confirmDelete)?.nom })}
              </p>
        ...
        {nettoyage.length === 0 && editingId !== 'NEW' && (
          <div className="text-center py-16 bg-gray-50 rounded-[2.5rem] border-2 border-dashed border-gray-200">
            ...
            <p className="text-gray-400 font-bold">{t('lbl_no_tasks')}</p>
            <Button onClick={handleCreate} variant="outline" className="mt-4">{t('btn_add_task')}</Button>
          </div>
        )}
      </div>
    </div>
